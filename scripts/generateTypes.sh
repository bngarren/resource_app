...
PATH_TO_OPENAPI=./src/openapi.json
...
echo
echo "This script is going to use an OpenAPI spec ($PATH_TO_OPENAPI) to generate backend types and frontend"
...
cd $PATH_TO_PROJECT && npx openapi-typescript $PATH_TO_OPENAPI --output $PATH_TO_BACKEND_TYPES --support-array-length true --prettier-config $PATH_TO_PRETTIERRC
echo
